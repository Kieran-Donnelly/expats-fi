...
import { articleSeoDescription, articleSeoTitle } from '@/lib/article-journeys'
...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const guideResults: SearchResult[] = articles.filter((article) => matches(query, articleSeoTitle(article.slug, article.title), articleSeoDescription(article.slug, article.description), article.category)).slice(0, 12).map((article) => ({ href: `/resources/${article.slug}/`, label: article.category, title: articleSeoTitle(article.slug, article.title), description: articleSeoDescription(article.slug, article.description), meta: `${article.readingMinutes} min read` }))
