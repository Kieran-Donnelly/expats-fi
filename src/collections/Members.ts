...
export const Members: CollectionConfig = {
  ...
  fields: [
    ...
    },
    {
      ...
      access: { update: ({ req: { user } }) => canManageMembers(user) },
      options: [
        { label: 'New member · review first', value: 'new' },
        { label: 'Trusted · clear posts may publish', value: 'trusted' },
        { label: 'Restricted · cannot post', value: 'restricted' },
      ],
      admin: { position: 'sidebar', description: 'Controls the community moderation lane. Change deliberately after reviewing the member’s contributions.' },
    },
    {
      name: 'communityRulesAcceptedAt',
      type: 'date',
      index: true,
      access: { update: ({ req: { user } }) => canManageMembers(user) },
    ...
    {
      name: 'communityRestrictionReason',
      type: 'textarea',
      maxLength: 1000,
      access: {
        read: ({ req: { user } }) => canManageMembers(user),
        update: ({ req: { user } }) => canManageMembers(user),
      },
      admin: { description: 'Private reason for restricting community posting. Never shown publicly.' },
    },
    {
      name: 'city',
      type: 'text',
