...
const storySlugs = seedNewsStories.map((story) => story.slug)
  ...
  for (const story of seedNewsStories) {
    ...
    await payload.create({
      ...
      data: {
        ...
        slug: story.slug,
        sourceCheckedAt: '2026-08-24T12:00:00.000Z',
...
export async function down({ payload, req }: MigrateDownArgs): Promise<void> {
  await payload.delete({ collection: 'news-stories', overrideAccess: true, req, where: { slug: { in: storySlugs } } })
}
