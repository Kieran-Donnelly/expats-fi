...
2. Work through Family, Housing, Community and Culture; verify mobile crop and text contrast on each template.
...
Unlimphotos was signed in during this review, but its download control failed with a page-side JavaScript error (`callbackCatch is not defined`). No preview/watermarked file was used as a substitute. Eleven distinct, free-to-use Unsplash photographs were added in this batch. The Start Here digital guide, all six Study guide pages, the Housing lease-and-moving-in guide and the first three Family guides received individual header images and matching social images. Their source records are in `docs/image-licences.md`. This leaves **33 editorial guide headers** and the community board landing header still to source and implement.
